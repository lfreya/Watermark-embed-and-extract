function [watermrkd_img,PSNR,NCC,MSSIM,recmessage,attack_image,attack_message,PSNR_a, NCC_a, MSSIM_a] = dwt(cover_object,message,var)
h=msgbox('Processing');
...
struct(1,1).data=input(:,:,1); %inputr
struct(2,1).data=input(:,:,2); %inputg
struct(3,1).data=double(input(:,:,3)); %inputb
struct(1,2).data=water(:,:,1); %waterr
struct(2,2).data=water(:,:,2); %waterg
struct(3,2).data=double(water(:,:,3)); %waterb

%系数r 大，鲁棒性增加，r小，透明性增加
r=0.04;
%水印R的分解
% [Cwr,Swr]=wavedec2(waterr,1,'haar');
[Cwr,Swr]=wavedec2(struct(1,2).data,1,'haar');
%图像R的分解
% [Cr,Sr]=wavedec2(inputr,2,'haar');
[Cr,Sr]=wavedec2(struct(1,1).data,2,'haar');
%水印的嵌入
...
[Cwg,Swg]=wavedec2(struct(2,2).data,1,'haar');
%图像G的分解
% [Cg,Sg]=wavedec2(inputg,2,'haar');
[Cg,Sg]=wavedec2(struct(2,1).data,2,'haar');
%水印的嵌入
...
[Cwb,Swb]=wavedec2(struct(3,2).data,1,'haar');
%图像B的分解
% [Cb,Sb]=wavedec2(inputb,2,'haar');
[Cb,Sb]=wavedec2(struct(3,1).data,2,'haar');
%水印的嵌入
...
% NCC=ncc(double(message1),double(recmessage));
